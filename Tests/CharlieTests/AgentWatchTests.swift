...
enum AgentWatchTests {
    ...
    static func parse() {
        Check.run("parses charlie-native omp line") {
        ...
        Check.run("charlie 'gone' removes") {
    ...
    static func installers() {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("charlie-watch-tests-\(UUID().uuidString)")
        ...
        Check.run("claude merge preserves existing settings") {
            ...
            Check.ok(out.contains("charlie-agent-watch"), "added ours")
            ...
            let markers = again.components(separatedBy: "charlie-agent-watch").count - 1
