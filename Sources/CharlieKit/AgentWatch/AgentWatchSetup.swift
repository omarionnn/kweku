...
///   that streams lifecycle events to Charlie's socket from inside the omp
...
public struct AgentWatchSetup {
    public static let socketPath = ("~/Library/Application Support/Charlie/agent.sock" as NSString)
    ...
    public var ompExtensionFile: URL { ompExtensionsDir.appendingPathComponent("charlie-watch.ts") }
    ...
    // charlie-watch.ts — installed by Charlie.app (agent watcher).
    // Streams omp session lifecycle to Charlie's notch creature. Fails silent:
    // it must never break a coding session.
    import * as net from "node:net";
    import * as os from "node:os";
    import * as path from "node:path";

    const SOCK = path.join(os.homedir(), "Library/Application Support/Charlie/agent.sock");
    ...
    static let claudeMarker = "charlie-agent-watch"
    ...
    try:
        ...
        s.connect(os.path.expanduser("~/Library/Application Support/Charlie/agent.sock"))
