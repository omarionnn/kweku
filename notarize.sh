#!/usr/bin/env bash
# Developer ID signing + notarization + stapling for the Kweku .app bundle.
#
...
APPDIR="${1:-build/Kweku.app}"
ENTITLEMENTS="Resources/Kweku.entitlements"
...
To notarize on a machine that does, set:
  ...
  export NOTARY_PROFILE="kweku"     # from `xcrun notarytool store-credentials`

then re-run: ./notarize.sh build/Kweku.app
EOF
  exit 2
fi
